...
export class VisualizationOneDataHandler {
  ...
  constructor(private view: View) {
    ...
    this.view.addSignalListener(
      'data_variable_selection',
      (_name, data: { LABEL: string[] }) =>
        this.updateDataVariable(
          data.LABEL && data.LABEL.length > 0 ? data.LABEL[0] : undefined
        )
    );
  }
  ...
  async setData(): Promise<void> {
    ...
    Object.entries(dataVariableMapping).forEach(([label, dataVariable]) => {
      this.subsets[label] = data.filter(
        (row) => row['DATA_VARIABLE'] === dataVariable
      );
    });
